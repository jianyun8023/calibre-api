...
**v1.x 的问题**：
- MeiliSearch（关键词搜索）+ Milvus（向量搜索）= 双重维护成本
- 数据同步复杂，需要维护两套索引
- 存储冗余，元数据重复存储

**v2.0 的优势**：
- **统一存储**：向量 + 元数据存储在同一个 Qdrant Collection
- **简化架构**：移除 MeiliSearch 和 Milvus，降低运维复杂度
...
#### Qdrant vs MeiliSearch 对比

| 特性 | Qdrant | MeiliSearch (v1.x) |
|------|--------|-------------------|
| **搜索类型** | 向量 + 标量 | 仅全文搜索 |
| **语义理解** | ✅ 优秀 | ❌ 不支持 |
| **关键词搜索** | ✅ 支持（Scroll + 过滤） | ✅ 优秀（BM25） |
| **混合搜索** | ✅ 原生支持 | ❌ 需要外部向量库 |
| **存储效率** | ✅ 统一存储 | ❌ 需要双重存储 |
| **索引速度** | 170本/秒 | 300本/秒 |
| **查询延迟** | < 50ms (P95) | < 30ms |
...
**结论**：Qdrant 在语义搜索方面有压倒性优势，虽然全文搜索略逊于 MeiliSearch，但统一架构带来的收益远大于性能差异。
