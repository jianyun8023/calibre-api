...
func NewMCPServer(api *Api, config MCPConfig) *MCPServer {
	...
	if config.Transport == "sse" || config.Transport == "" {
		...
		log.Info("SSE transport initialized")
...
func (m *MCPServer) Mount(r *gin.Engine) {
	if m.sseServer != nil {
		...
		// 使用 SSEServer 的 ServeHTTP 方法（它实现了 http.Handler）
		r.GET(sseEndpoint, gin.WrapH(m.sseServer))
		r.POST(messageEndpoint, gin.WrapH(m.sseServer))
